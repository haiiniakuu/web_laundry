...
class TransOrderController extends Controller
{
    ...
    {
         $taxRate = 0.10;
    $taxAmount = $order->total * $taxRate;
    $totalWithTax = $order->total + $taxAmount;
        $request->validate([
        'order_pay' => 'required|numeric|min:' . $totalWithTax,
    ]);

        $order->order_pay = $request->order_pay;
        $order->order_change = $request->order_pay - $totalWithTax;
